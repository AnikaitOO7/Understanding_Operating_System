...
int main(){
    ...
    array = burst;
    qsort(index, p, sizeof(*index), compare);
...
int sjf_table(int burst[]){
	...
	for(i=0; i < p ;++i)
    {
        printf("\n\t  P%d \t\t\t    %d \t\t\t\t       %d \t\t\t\t %d \t\t\t\t      %d \t\t\t       %d\n", index[i]+1, burst[index[i]], com[i], com[i], com_1[i], com_1[i]);
    }
    line(175);
